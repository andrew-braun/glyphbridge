# Greek Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `greek-reading-v1` should move
...
- Should final sigma be represented as a distinct `curriculum.graphemes` row, a
  variant of sigma in `course_version_graphemes.details`, or an
  `orthography_rules` entry linked to examples?
- Should digraphs such as `ει` and `ου` be stored as graphemes, reusable chunks,
  or orthography-rule examples?
- How should uppercase/lowercase case pairs be represented for sign-heavy lesson
  examples?

...
create or replace function internal_api.set_updated_at()
returns trigger
language plpgsql
...
create or replace function internal_api.handle_new_user()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
	if tg_op = 'UPDATE' then
		if old.status = 'published'::public.course_version_status then
			if (to_jsonb(new) - 'status') is distinct from (to_jsonb(old) - 'status') then
				raise exception 'Published course versions are immutable';
			end if;

			if new.status not in ('published'::public.course_version_status, 'archived'::public.course_version_status) then
				raise exception 'Published course versions may only transition to archived';
			end if;
		elsif old.status = 'archived'::public.course_version_status then
	...
	if old.status in ('published'::public.course_version_status, 'archived'::public.course_version_status) then
...
drop trigger if exists prevent_course_versions_mutation on curriculum.course_versions;

create trigger prevent_course_versions_mutation
	before update or delete on curriculum.course_versions
	for each row
	execute function internal_api.prevent_course_version_mutation();

drop function if exists internal_api.sync_lesson_attempt_batch(uuid, uuid, uuid, jsonb);
...
alter table curriculum.courses
	drop constraint if exists courses_current_published_version_fkey;

alter table curriculum.courses
	drop constraint if exists courses_current_published_version_same_course_fkey;
...
alter table curriculum.course_versions
	drop constraint if exists course_versions_created_by_fkey;
...
alter table delivery.course_publications
	drop constraint if exists course_publications_created_by_fkey;

alter table delivery.course_publications
	add constraint course_publications_created_by_fkey
	foreign key (created_by)
	references auth.users(id)
	on delete set null;

alter table learner.profiles
...
alter table learner.devices
	drop constraint if exists devices_client_device_id_length_check,
	add constraint devices_client_device_id_length_check
	check (length(client_device_id) between 1 and 128),
	drop constraint if exists devices_platform_length_check,
	add constraint devices_platform_length_check
	check (platform is null or length(platform) <= 64);

alter table learner.lesson_attempts
	drop constraint if exists lesson_attempts_client_attempt_id_length_check,
	add constraint lesson_attempts_client_attempt_id_length_check
	check (length(client_attempt_id) between 1 and 64),
	drop constraint if exists lesson_attempts_time_spent_ms_upper_bound_check,
	add constraint lesson_attempts_time_spent_ms_upper_bound_check
	check (time_spent_ms is null or time_spent_ms <= 86400000),
	drop constraint if exists lesson_attempts_attempt_payload_size_check,
	add constraint lesson_attempts_attempt_payload_size_check
	check (octet_length(attempt_payload::text) <= 32768);

alter table curriculum.languages enable row level security;
alter table curriculum.script_systems enable row level security;
alter table curriculum.courses enable row level security;
alter table curriculum.course_versions enable row level security;
alter table curriculum.graphemes enable row level security;
alter table curriculum.course_version_graphemes enable row level security;
alter table curriculum.orthography_rules enable row level security;
alter table curriculum.orthography_rule_examples enable row level security;
alter table curriculum.lessons enable row level security;
alter table curriculum.anchor_targets enable row level security;
alter table curriculum.anchor_segments enable row level security;
alter table curriculum.lesson_graphemes enable row level security;
alter table curriculum.lesson_rules enable row level security;
alter table curriculum.drills enable row level security;
alter table curriculum.drill_options enable row level security;
alter table curriculum.lesson_drills enable row level security;

revoke all on learner.profiles from authenticated;
grant select, update on learner.profiles to authenticated;

revoke all on learner.devices from authenticated;
grant select on learner.devices to authenticated;

revoke all on learner.lesson_attempts from authenticated;
grant select on learner.lesson_attempts to authenticated;

revoke all on learner.preferences from authenticated;
grant select, update on learner.preferences to authenticated;

drop policy if exists learner_profiles_insert_own on learner.profiles;
drop policy if exists learner_devices_insert_own on learner.devices;
drop policy if exists learner_devices_update_own on learner.devices;
drop policy if exists learner_devices_delete_own on learner.devices;
drop policy if exists learner_lesson_attempts_insert_own on learner.lesson_attempts;
drop policy if exists learner_preferences_insert_own on learner.preferences;
